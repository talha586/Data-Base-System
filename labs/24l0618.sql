-- Replace [YourName_RollNo] with your actual details
USE [24l0618];
...
--Q1
SELECT fname+' ' + lname AS NAMES FROM doctor WHERE salary > (SELECT AVG (salary) FROM doctor)

--Q2
SELECT fname+' ' + lname AS NAMES FROM doctor WHERE ssn IN (SELECT essn FROM performed_by) 

--Q3
SELECT fname+' ' + lname AS NAMES FROM doctor d WHERE salary>(SELECT AVG(salary) FROM doctor doc WHERE d.sno=doc.sno)

--Q4
SELECT AVG(hours) AS TIMES FROM performed_by WHERE essn IN (SELECT essn FROM performed_by GROUP BY essn HAVING SUM(hours)>15)

--Q5
SELECT sname AS specialization_name FROM specialization WHERE snumber IN(SELECT sno FROM doctor GROUP BY sno HAVING COUNT(ssn)>2)

--Q6
SELECT sname FROM specialization WHERE mgrssn NOT IN (SELECT DISTINCT essn FROM performed_by)

--Q7
SELECT fname+' '+lname FROM doctor WHERE ssn IN (SELECT essn FROM performed_by WHERE hours >  (SELECT AVG(hours) FROM performed_by))

--Q8
SELECT sno , COUNT(ssn) AS Numbers FROM doctor d WHERE salary > (SELECT salary FROM doctor doc WHERE d.superssn=doc.ssn) GROUP BY sno

--Q9
SELECT sname FROM surgery WHERE snumber IN (SELECT sno FROM performed_by WHERE hours > ALL (SELECT hours FROM performed_by WHERE sno IN(SELECT snumber FROM surgery WHERE sname='Oncology')))

--Q10
SELECT d.fname + ' ' + d.lname AS Doctor_Name FROM doctor d WHERE (SELECT COUNT(*)  FROM dependent dep  WHERE dep.essn = d.ssn) > 1 AND d.ssn IN (SELECT pb1.essn FROM performed_by pb1 WHERE pb1.hours > 2 * ( SELECT AVG(pb2.hours) FROM performed_by pb2 WHERE pb2.sno = pb1.sno AND pb2.essn <> pb1.essn))
